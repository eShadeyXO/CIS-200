...
//Program 2
//This class contains functionality for the program two letter form that is called from the main form.
using System;
    ...
    public partial class LetterForm : Form
    {
        ...
        private void originCombo_Validating(object sender, CancelEventArgs e)
        {
            ...
            else if (originCombo.SelectedIndex == destCombo.SelectedIndex)
            {
                e.Cancel = true;
                originCombo.Focus();
                originErrorProvider.SetError(originCombo, "Origin and destination address can not be the same.");
            }
        }
        ...
        private void destCombo_Validating(object sender, CancelEventArgs e)
        {
            if (destCombo.SelectedIndex < 0)
            {
                e.Cancel = true;
                destCombo.Focus();
                destErrorProvider.SetError(destCombo, "Must select a destination address.");
            }
            else if (destCombo.SelectedIndex == originCombo.SelectedIndex)
            {
                e.Cancel = true;
                destCombo.Focus();
